export function scoreClaim(claim) {
  ...
  const completeness = Math.min(100, fields.filter(Boolean).length * 12 + (claim.consent ? 4 : 0));
  const confidence = score >= 75 && flags.length <= 1 ? "High" : score >= 45 ? "Medium" : "Low";
...
function suggestUse(claim) {
  ...
  if ((claim.evidenceUrl || "").trim() && (claim.location || "").trim()) {
    return "Useful for AI Training";
  }
